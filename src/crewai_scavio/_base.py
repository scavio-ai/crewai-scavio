"""Base class for all Scavio CrewAI tools."""
...
class ScavioBaseTool(BaseTool):
    ...
    def _truncate_nested(
        self, raw: dict[str, Any], outer: str, inner: str
    ) -> dict[str, Any]:
        """Truncate a nested list field (e.g., data.products) to max_results."""
        data = raw.get(outer)
        if isinstance(data, dict):
            items = data.get(inner)
            if isinstance(items, list) and len(items) > self.max_results:
                raw[outer][inner] = items[: self.max_results]
        elif isinstance(data, list) and len(data) > self.max_results:
            raw[outer] = data[: self.max_results]
        return raw
